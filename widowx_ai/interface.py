...
class ModelTestRunner:
    def __init__(self, project_root: Path, controller: ArmController) -> None:
        ...
        self.script = project_root / "widowx_ai" / "run_act_safe.py"
        self.lock = threading.Lock()
    ...
    def run(self, payload: dict[str, Any]) -> dict[str, Any]:
        ...
        if not self.script.exists():
            raise RuntimeError(f"Model test script not found: {self.script}")
...
def main() -> int:
    ...
    controller = ArmController(args, Path(__file__).resolve().parent / "start_position.json")
    camera_controller = CameraController()
